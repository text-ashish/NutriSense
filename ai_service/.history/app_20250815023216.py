...
if st.button("Get Personalized Recommendation"):
    if not user_description.strip():
        ...
        # Construct final query for retrieval
        filters_str = []
        ...
        st.markdown(f"🔍 **Constructed Query for RAG:** `{final_query}`")
        ...
        if not results:
            ...
            # Transparency: Show retrieved recipes
            with st.expander("📖 Retrieved Recipes (Transparency)"):
                for r in results:
                    st.markdown(f"### {r.get('recipe_name', 'Unnamed')}")
