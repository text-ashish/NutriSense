...
def retrieve(
    ...
    for idx, (meta, doc) in enumerate(zip(query_result["metadatas"][0], query_result["documents"][0])):
        ...
        # ---------------------------
        # ✅ Dietary filter (robust)
        # ---------------------------
        if dietary_filter_norm:
            try:
                labels = normalize_labels_list(recipe.get("dietary_labels", []))
                # Include recipe only if it explicitly matches the user preference
                if dietary_filter_norm not in labels:
                    continue
            except:
                continue
        ...
        if exclude_allergens:
            try:
                allergens_list = json.loads(recipe.get("allergens", "[]")) if isinstance(recipe.get("allergens"), str) else recipe.get("allergens", [])
        ...
        if health_condition:
            try:
                tags = json.loads(recipe.get("health_tags", "[]")) if isinstance(recipe.get("health_tags"), str) else recipe.get("health_tags", [])
