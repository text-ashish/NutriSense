...
from src.embeddings import get_embeddings  # Make sure you have this function
...
def build_vectorstore(df, embeddings):
    ...
    Convert all metadata values to Chroma-supported types (str, int, float, bool).
    None values will be replaced with empty strings or 0.
    """
...
def retrieve(
    query_embedding=None, query_text=None, k=5,
    ...
    for i in range(len(results['ids'][0])):
        ...
        # Nutrition goals filtering
        nutrition_data = {}
        try:
            nutrition_data = json.loads(recipe_meta.get('nutrition_normalized', '{}'))
        except:
            pass

        if calorie_goal and nutrition_data.get('calories', 0) > calorie_goal:
            continue
        if protein_goal and nutrition_data.get('protein_g', 0) < protein_goal:
            continue
        if carb_goal and nutrition_data.get('carbohydrates_g', 0) > carb_goal:
            continue
        if fat_goal and nutrition_data.get('fat_g', 0) > fat_goal:
            continue
