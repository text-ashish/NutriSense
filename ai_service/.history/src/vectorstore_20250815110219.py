...
def retrieve(query_embedding, k=6, dietary_filter=None, exclude_allergens=None,
    ...
    # Retrieve from vector store first (whatever logic you already have)
    results = collection.query(
        query_embeddings=[query_embedding],
        n_results=k
    )  # Assuming this returns list of dicts
    ...
    if exclude_allergens:
        results = [
            r for r in results
            if not any(allergen.lower() in r.get("ingredients", "").lower() for allergen in exclude_allergens)
        ]

    # ✅ Health condition filter (if you have logic for this)
    if health_condition and health_condition != "None":
        # Example — adapt based on your data
        results = [
            r for r in results
