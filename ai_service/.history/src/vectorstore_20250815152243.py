...
def parse_time(value):
    """Convert '10 min', '1 hr 20 min', or numeric values to int minutes."""
    if pd.isna(value):
        return None
...
def build_vectorstore(df, embeddings):
    ...
    for idx, emb in enumerate(embeddings):
        metadata = {
            ...
            "prep_time": parse_time(df.get('prep_time', [0])[idx]),
            "cook_time": parse_time(df.get('cook_time', [0])[idx]),
            "total_time": parse_time(df.get('total_time', [0])[idx]),
