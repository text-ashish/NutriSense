...
def retrieve(
    ...
    query_result = collection.query(
        ...
        n_results=k,
        include=["metadatas", "documents"]  # removed "ids" to avoid ValueError
    )

    results = []
    for idx, (meta, doc) in enumerate(zip(query_result["metadatas"][0], query_result["documents"][0])):
        recipe_id = query_result["ids"][0][idx]  # still accessible without include
        recipe = meta
        recipe["document"] = doc
        recipe["id"] = recipe_id
        ...
        if dietary_filter and recipe.get("dietary_labels"):
            try:
                ...
                if dietary_filter.lower() == "vegetarian" and "non vegetarian" in labels_lower:
                    continue
                if dietary_filter.lower() == "non veg" and "vegetarian" in labels_lower:
                    continue
            except:
                pass
